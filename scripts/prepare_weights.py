...
def pre_compile_edge_yolo():
    ...
    print("🔥 Starting clean native Ultralytics FP16 compilation...")
    model = YOLO(model_path)
    
    # Let Ultralytics safely export with native graph safety rules
    model.export(format='onnx', quantize=16, simplify=True, device=0)
    
    if os.path.exists(expected_output):
        file_size_mb = os.path.getsize(expected_output) / (1024 * 1024)
        print(f"✅ Success: Native edge weights compiled -> {expected_output}")
        print(f"📦 Graph File Size: {file_size_mb:.2f} MB (Perfect FP16 size)")
